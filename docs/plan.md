...
**Última actualización:** 2026-08-25 — v1.1 agrega despliegue híbrido, app móvil mínima, dimensioning y repack & consolidation (nueva Fase 3, ver Sección 6).
...
La app móvil y el modelo de despliegue híbrido (Fase 3) no son Bounded Contexts: son, respectivamente, un canal de acceso adicional y una topología de despliegue sobre los mismos contextos ya definidos.
...
| **Fase 3** | Plataforma Híbrida, App Móvil, Dimensioning y Repack & Consolidation | 6-8 semanas | Despliegue on-premise validado además del de nube, app móvil mínima en producción, integración con al menos un modelo de báscula/dimensionador, flujo de repack/consolidation operativo en WMS |
...
**Duración total estimada: 20-24 meses** con el modelo de ejecución propuesto (desarrollador senior + asistencia de IA). El incremento de ~6-8 semanas frente a la versión anterior del plan corresponde a la nueva Fase 3.

> El orden de las fases (Forwarding → WMS → Plataforma Híbrida/Móvil/Dimensioning/Repack → CRM → Accounting → Customs) puede reordenarse según qué área genera más impacto inmediato en la operación del cliente. Esto es una decisión conjunta con el cliente en la Fase 0. La Fase 3 se ubica después de WMS porque Dimensioning y Repack & Consolidation son capacidades de ese contexto (Sección 4.1) y requieren que `Package`/`Shipment` ya existan.
